...
prep_environment $scriptdir 'ARC'
prep_dataset $DATA/EM/Myrf_01/SET-B 'B-NT-S10-2f_ROI_00' '0500'
...
script_mpi_conda $scriptfile $(get_cmd_dm3convert)

export template='single' additions='mpi' CONDA_ENV=''
export njobs=1 nodes=4 tasks=16 memcpu=2000 wtime='01:00:00' q='h'
...
#
# subdir='tif'
# scriptfile="$datadir/EM_ds${subdir}_script.sh"
# script_mpi_conda $scriptfile $(get_cmd_downsample_slices)
#
# export template='single' additions='mpi' CONDA_ENV=''
...
export jobname='reg'
export cmd=$(get_cmd_fiji_register)
source $scriptdir/pipelines/template_job_$template.sh
...
script_mpi_conda $scriptfile $(get_cmd_series2stack)

export template='single' additions='' CONDA_ENV=''
export njobs=1 nodes=1 tasks=1 memcpu=30000 wtime='03:00:00' q=''
export jobname='series2stack'
export cmd=$scriptfile
source $scriptdir/pipelines/template_job_$template.sh


###=========================================================================###
### ilastik training data
###=========================================================================###
...
###=========================================================================###
# On LOCAL: sync the training data to LOCAL
...
mkdir -p $loc_datadir/blocks_$bs
fname="blocks_$bs/${dataset}_03000-03500_03000-03500_00000-`printf %05d ${Z}`.h5"
rsync -Pazv $rem_host:$rem_datadir/$fname $loc_datadir/blocks/

# On LOCAL: do the interactive classifier training
scriptdir="$HOME/workspace/EM"
export PYTHONPATH=$PYTHONPATH:$scriptdir
alias ilastik=/Applications/ilastik-1.2.2post1-OSX.app/Contents/MacOS/ilastik
ilastik &

# On LOCAL: send classifier to ARC
...
pixprob_trainingset="pixclass_8class"
ipf=; ids='data'; opf='_probs'; ods='volume/predictions'

export template='single' additions='' CONDA_ENV=''
export njobs=1 nodes=1 tasks=16 memcpu=125000 wtime='23:00:00' q=''
export jobname='apply_ilastik'
export cmd=$(get_cmd_apply_ilastik)
source $scriptdir/pipelines/template_job_$template.sh
